...
namespace TestSFML {
	...
	public:
		...
		bool hasCommonEdge(const Triangle<T> &t) const
		{
			return 
				(containsVertex(t.p1) && containsVertex(t.p2)) || 
				(containsVertex(t.p2) && containsVertex(t.p3)) ||
				(containsVertex(t.p1) && containsVertex(t.p3)) ;
		}
		...
		VertexType circumCircleCenter() const
		{
			const T ab = p1.norm2();
			const T cd = p2.norm2();
			const T ef = p3.norm2();
		...
		////TODO make better
		//T circumCircleCenterRadius() const
		//{
		//	const T ab = p1.norm2();
		//	const T cd = p2.norm2();
		//	const T ef = p3.norm2();
